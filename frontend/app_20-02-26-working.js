...
function makePageWrap(pg, b64, wPx, side) {
    ...
    pg.changes.forEach(ch => {
        ...
        hl.addEventListener('mouseenter', () => {
            ...
            // Show character-level overlay on hover
            const overlay = hl.querySelector('.text-diff-overlay');
            if (overlay) {
...
function selectChange(id, updateIdx = true, showDetail = false) {
    ...
    document.querySelectorAll('.hl').forEach(el => {
        ...
        // Hide character overlay for inactive highlights
        const overlay = el.querySelector('.text-diff-overlay');
        if (overlay) {
            overlay.style.opacity = '0';
        }
    });
    ...
    document.querySelectorAll(`[data-id="${id}"]`).forEach(el => {
        ...
        // Show character overlay for active highlight
        const overlay = el.querySelector('.text-diff-overlay');
        if (overlay) {
            overlay.style.opacity = '1';
        }
    ...
    if (ch.type === 'text' && ch.word_diff && ch.word_diff.length > 0) {
        ...
        if (ch.old_text || ch.new_text) {
            ...
            if (ch.old_text && ch.change !== 'added') {
                ...
                contentHtml += `<div style="padding: 8px; background: #FFEBEE; border-left: 3px solid #F44336; border-radius: 4px; font-size: 12px; color: #161b27;">${esc(ch.old_text)}</div>`;
                contentHtml += '</div>';
            }
            ...
            if (ch.new_text && ch.change !== 'deleted') {
                ...
                contentHtml += `<div style="padding: 8px; background: #E8F5E9; border-left: 3px solid #4CAF50; border-radius: 4px; font-size: 12px; color: #161b27;">${esc(ch.new_text)}</div>`;
                contentHtml += '</div>';
            }
