...
function renderPages(pages) {
    ...
    // Search UI is now in HTML, no need to add programmatically
    // addSearchUI();
...
function renderPageSelector(pages) { // Create page selector bar (find or create container)
    ...
    label.style.cssText = 'color: #888; font-size: 12px; margin-right: 5px; white-space: nowrap;';
    label.textContent = 'Page:';
    selector.appendChild(label);

    // Create left arrow button (only if many pages)
    if (showArrows) {
        const leftArrow = document.createElement('button');
        leftArrow.id = 'pageSelectorLeftArrow';
        leftArrow.innerHTML = '◀';
    ...
    // Create scrollable container for page buttons
    const scrollContainer = document.createElement('div');
    scrollContainer.id = 'pageSelectorScroll';
    ...
    if (!document.getElementById('pageSelectorScrollStyle')) {
        style.id = 'pageSelectorScrollStyle';
    ...
    pages.forEach(pg => {
        ...
        btn.className = 'page-selector-btn';
        btn.dataset.page = pg.page_num;
    ...
    selector.appendChild(scrollContainer);

    // Create right arrow button (only if many pages)
    if (showArrows) {
        const rightArrow = document.createElement('button');
        rightArrow.id = 'pageSelectorRightArrow';
...
// Helper function to scroll the page selector
function scrollPageSelector(amount) {
...
function updateSearchUI() {
    const counter = document.getElementById('searchResultsLabel'); // Changed from searchCounter
    ...
    // Note: Prev/Next buttons in HTML don't need disabled state
}
...
function scrollToPage(pageNum, bbox) {
    ...
    console.log(`[Scroll] Jumping to page ${pageNum}, position: ${targetScrollPos}`);
...
function jumpToPage(pageNum) {
    ...
    if (pageNum < 1 || pageNum > diffData.pages.length) {
        ...
        showToast(`Page ${pageNum} does not exist`);
        return;
    }
